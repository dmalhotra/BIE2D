...
namespace sctl {
  ...
  template <class Real, Integer Order = 16> class DiscMobility : public ICIP<Real,Order> {
    ...
    DiscMobility(const Comm& comm_ = Comm::Self()) : ICIP_Base(comm_), StokesSL_BIOp(StokesSL_Ker, false, this->comm), StokesDL_BIOp(StokesDL_Ker, false, this->comm), solver(this->comm, true) {}
    ...
    void Solve(Vector<Real>& V, const Vector<Real> F, const Vector<Real> Vs, const Long gmres_max_iter) {
      ...
      if (0 && this->disc_panels.GetNearList().Dim()) { //////////////////////////////////////
